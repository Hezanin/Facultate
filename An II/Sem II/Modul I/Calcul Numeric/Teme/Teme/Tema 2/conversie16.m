...
if fix(x)>0 % verificam daca avem parte intreaga la numarul real x
 ...
 while fix(x1) > 0
 rest=mod(x1,16); % retin restul impartirii la 2
 x2=fix(x1/16); % retin catul lui x1/2
...
if (x-fix(x) > 0) % verificam daca avem parte fractionara la numarul real
...
if(ParteInt==1) % exista parte intreaga
 fprintf('Conversia in baza 16 pentru n=%d este:\n',n)
 if nrNeg==1
 fprintf('-'); % pentru ca avem un numar negativ va adauga '-'
 end
 for i=1:length(cifre_parte_intreaga)
    switch class(cifre_parte_intreaga(i))
        case 'char'
            fprintf('%c',cifre_parte_intreaga(i));
        case 'string'
            fprintf('%s',cifre_parte_intreaga(i));
        otherwise
...
if(ParteFract==1) % exista parte fractionara  
 ...
 for i=1:dimensiune_max
    switch class(cifre_parte_intreaga)
    case 'char'
        fprintf('%c',cifre_parte_intreaga);
    case 'string'
        fprintf('%s',cifre_parte_intreaga);
    otherwise
        fprintf('%d',cifre_parte_intreaga);
end
 end
 else
